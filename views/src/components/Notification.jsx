...
const Notification = () => {
  ...
  const newArrayForUserChatUpdate = friendReqSender?.map((friendReq) => {
    ...
    const receiverUsername = allUsers.find(item => item?.unique_id === receiverUserUniqueId[0]); // data profile user yg dikirim friend_req
          ...
          <details className="">
            ...
            {newArrayForUserChatUpdate && newArrayForUserChatUpdate?.length > 0 && newArrayForUserChatUpdate[0] !== null ? (
              <ul className="p-2 bg-base-200 rounded-t-none rounded-box w-56 absolute right-0 z-10">
                ...
                {/* <li>
                  <div className="flex">
                    <a className="text-xs">You have a message from <span className="font-bold">username</span></a>
                    <button className="btn btn-xs btn-primary" onClick={() => handleOnUserChatUpdate(true, null, user?.chat_unique_id)}>See</button>
                  </div>
                </li> */}
              </ul>
            ) : (
